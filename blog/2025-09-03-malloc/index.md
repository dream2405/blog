...
최소 double-word 정렬이 강제되는 이유는, `long double`이나 SIMD 벡터가 단일 워드를 넘어서는 데이터 단위를 처라하기 때문이라고 한다.
...
- 1040 바이트 이하 크기의 청크에서는 free를 해도 다음 청크의 P 플래그가 0이 되지 않는다.
    - 1040 바이트 이하 크기의 청크는 fastbin에 들어간다고 추측됨
...
```
`fd`, `bk`, `fd_nextsize`, `bk_nextsize` 포인터들은 **빈(bin)**의 구현에 사용된다.
